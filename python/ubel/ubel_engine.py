...
* dependencies_tree           — nested dict for the HTML graph (legacy, not used in UI)
...
def build_dependency_tree(inventory: List[Dict]) -> Dict:
    """
    Build a nested dict tree used by the HTML graph renderer (legacy, kept for
    compatibility but not used in the current UI). Only nodes that are
    'vulnerable' or 'infected' are included, plus every ancestor.
    """
    by_id: Dict[str, Dict] = {c["id"]: c for c in inventory}
...
# ---------------------------------------------------------------------------
# HTML report  (client-side JS modified to show dependency sequences via autocomplete)
# ---------------------------------------------------------------------------
...
<head>
    ...
    <style>
        ...
        .seq-package { transition: all 0.2s; }
        .seq-package:hover { background: #2d2d2d; transform: translateY(-1px); }
    </style>
</head>
<body class="min-h-screen flex flex-col">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
    ...
    <script>
        ...
        // --- Existing functions (unchanged except graph init removal) ---
        function init() {
            closeModal();
            renderDashboard();
        ...
        function switchTab(tabId) {
            ...
            // No graph initialization needed
        }
    ...
    def scan(self,
        ...
        *,
        ...
        try:
            ...
            try:
                    from .reachability_analyzer import enrich_report as enrich_reachability
            except ImportError:
                    enrich_reachability = None
            if enrich_reachability is not None:
                try:
                    enrich_reachability(final_json, project_root=start_dir)
                except Exception as _e:
                    import sys as _sys
                    print(f"[~] Reachability failed (non-fatal): {_e}", file=_sys.stderr)
            ...
            if save_reports:
                ...
                latest_dir = Path(f"{self.current_dir}/.ubel") / "reports"
