...
class JavaMavenScanner:
    ...
    @staticmethod
    def _maven_purl(group_id: str, artifact_id: str, version: str) -> str:
    ...
    @staticmethod
    def _is_maven_root(directory: str) -> bool:
    ...
    @staticmethod
    def _extract_tag(xml: str, tag: str) -> List[str]:
        return re.findall(
    ...
    @staticmethod
    def _parse_pom_deps(xml: str) -> List[Dict[str, str]]:
        deps: List[Dict[str, str]] = []
        for block_m in re.finditer(r"<dependency>([\s\S]*?)</dependency>", xml, re.IGNORECASE):
            block = block_m.group(1)
            group_id    = (JavaMavenScanner._extract_tag(block, "groupId")    or [""])[0]
            artifact_id = (JavaMavenScanner._extract_tag(block, "artifactId") or [""])[0]
            version     = (JavaMavenScanner._extract_tag(block, "version")    or [""])[0]
            scope       = (JavaMavenScanner._extract_tag(block, "scope")      or ["compile"])[0]
            optional    = (JavaMavenScanner._extract_tag(block, "optional")   or ["false"])[0]
    ...
    @staticmethod
    def _parse_pom_properties(xml: str) -> Dict[str, str]:
        props: Dict[str, str] = {}
    ...
    @staticmethod
    def _resolve_version(version: str, props: Dict[str, str]) -> str:
        if not version:
            return ""
        resolved = re.sub(
            r"\$\{([^}]+)\}",
    ...
    @staticmethod
    def _parse_deps_tree(file_path: str) -> List[Dict[str, str]]:
        deps: List[Dict[str, str]] = []
        try:
    ...
    @staticmethod
    def _parse_modules(xml: str) -> List[str]:
        mods: List[str] = []
    ...
    @staticmethod
    def _maven_scope_to_ubel(mvn_scope: str) -> str:
    ...
    @classmethod
    def _scan_project(cls, project_root: str, visited: Set[str]) -> List[Dict[str, Any]]:
        ...
        props      = cls._parse_pom_properties(xml)
        components: List[Dict[str, Any]] = []

        # Prefer pre-generated dependency tree
        tree_file = os.path.join(project_root, ".ubel", "maven-deps.txt")
        tree_deps = cls._parse_deps_tree(tree_file)

        if tree_deps:
            deps = tree_deps
        else:
            deps = [
                {**d, "version": cls._resolve_version(d["version"], props)}
                for d in cls._parse_pom_deps(xml)
            ]
        ...
        for dep in deps:
            ...
            if version == "unknown":
                version = ""
            cid   = cls._maven_purl(dep["groupId"], dep["artifactId"], version)
            scope = cls._maven_scope_to_ubel(dep.get("scope") or "compile")
        ...
        for mod in cls._parse_modules(xml):
            mod_root = os.path.join(project_root, mod)
            components.extend(cls._scan_project(mod_root, visited))
    ...
    def get_installed(cls, start_dir: str = ".") -> List[str]:
        ...
        if cls._is_maven_root(start_dir):
            raw.extend(cls._scan_project(start_dir, visited))
            ...
            with entries as it:
                for entry in it:
                    ...
                    if entry.name in skip:
                        continue
                    full = entry.path
                    if cls._is_maven_root(full):
                        raw.extend(cls._scan_project(full, visited))
